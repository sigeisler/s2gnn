...
def extended_optim_cfg(cfg):
    ...
    # To apply model averaging, set 'ema' or 'swa
    cfg.optim.model_averaging = None

    # Epoch in which model averaging shall start
    cfg.optim.model_averaging_start = 0

    # If true do not apply weight decay to last layer
    cfg.optim.last_layer_no_wd = False

    # Number of epochs to alternate between spatial and spectra message passing
    cfg.optim.quasi_alternating = -1

    # Patience for early stopping (no improvement on validation loss)
    cfg.optim.stop_patience = 1_000
