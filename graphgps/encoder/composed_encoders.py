...
from graphgps.encoder.maglap_pos_encoder import MagLapPENodeEncoder
...
from graphgps.encoder.dummy_node_encoder import DummyNodeEncoder
from graphgps.encoder.linear_node_encoder import LinearNodeEncoder
from graphgps.encoder.ogbn_arxiv_encoder import OGBNArxivLinearNodeEncoder
...
ds_encs = {'Atom': AtomEncoder,
           ...
           'OGBNArxivLinearNode': OGBNArxivLinearNodeEncoder}
...
# Combine both MagLapPE and RWSE positional encodings.
for ds_enc_name, ds_enc_cls in ds_encs.items():
    register_node_encoder(
        f"{ds_enc_name}+MagLapPE+RWSE",
        concat_node_encoders(
            [ds_enc_cls, MagLapPENodeEncoder, RWSENodeEncoder],
            ['MagLapPE', 'RWSE'])
    )
