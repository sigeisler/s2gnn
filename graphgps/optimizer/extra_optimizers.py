...
from torch.optim import Adagrad, AdamW, Optimizer, RMSprop
...
@register.register_scheduler('constant_with_warmup')
def constant_with_warmup_scheduler(optimizer: Optimizer,
...
def create_optimizer(params: Union[List[Dict[str, Any]],
                                   Iterator[Parameter]], cfg: Any) -> Any:
    r"""Creates a config-driven optimizer."""
    if isinstance(params, list) and params and isinstance(params[0], dict):
        for param_dict in params:
            param_dict['params'] = filter(
                lambda p: p.requires_grad, param_dict['params'])
    else:
