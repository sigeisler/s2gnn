...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    if 'MagLapPE' in pe_types:
        k = cfg.posenc_MagLapPE.max_freqs
        ...
        # Check if precomputed eigenvectors are available and match the config.
        if (hasattr(data, 'laplacian_eigenvector_plain') and
                hasattr(data, 'laplacian_eigenvalue_plain') and
                hasattr(data, 'laplacian_config') and
                data.laplacian_config == transform.__repr__() and
                data.laplacian_eigenvalue_plain.shape[-1] >= k):
            # Use precomputed eigenvectors, truncated to k eigfreqs
            for attr in ['laplacian_eigenvector_plain',
                         'laplacian_eigenvalue_plain',
                         'laplacian_eigenvalue_plain_mask',
                         'laplacian_eigenvector_plain_posenc']:
                if hasattr(data, attr):
                    setattr(data, attr, getattr(data, attr)[..., :k])
        else:
            # Delete precomputed eigenvectors if necessary
            for attr in ['laplacian_eigenvector_plain',
                         'laplacian_eigenvalue_plain',
                         'laplacian_eigenvalue_plain_mask',
                         'laplacian_eigenvector_plain_posenc']:
