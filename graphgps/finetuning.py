...
def load_pretrained_model_cfg(cfg):
    ...
    if not os.path.isfile(pretrained_cfg_fname):
        logging.warning(f"Config not found: {pretrained_cfg_fname}")
        return cfg
...
def init_model_from_pretrained(model, pretrained_dir,
    ...
    if pretrained_dir.endswith('ckpt'):
        ckpt_file = pretrained_dir
    else:
        ckpt_file = get_final_pretrained_ckpt(
            osp.join(pretrained_dir, '0', 'ckpt'))
    ...
    if reset_prediction_head:
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           ...
                           and not k.startswith('model.post_mp')}
    pretrained_dict = {
    ...
    extra_pretrained = pretrained_keys - model_keys
    if extra_pretrained:
        print(f'Pretrained model contains unexpected keys {extra_pretrained}')
    extra_model = model_keys - pretrained_keys
    if extra_model:
        print(f'Loaded model contains unexpected keys {extra_model}')
